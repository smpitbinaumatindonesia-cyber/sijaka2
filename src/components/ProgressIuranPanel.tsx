  ...
  return (
    <div className="bg-[#0B1428] border border-slate-800/80 rounded-2xl p-5 sm:p-6 shadow-sm relative overflow-hidden h-full flex flex-col justify-between">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-800/80">
        ...
        {/* Year Selector Pills */}
        <div className="flex items-center gap-1 bg-slate-900 p-0.5 rounded-lg border border-slate-800 text-xs font-semibold self-start sm:self-auto">
      ...
      <div className="space-y-4 my-auto pt-3">
        ...
        {/* Summary Block */}
        <div className="flex items-center gap-4 p-3.5 bg-slate-900/60 rounded-xl border border-slate-800/80">
          {/* Circular Indicator */}
          ...
          <div className="space-y-0.5">
            ...
            <div className="text-[10px] text-slate-400">
              Periode Pembukuan Tahun {selectedYear}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
          {monthsStatus.map((m) => (
            <div
              key={m.label}
              className={`flex items-center justify-center gap-1 py-1.5 px-2 rounded-lg border text-xs font-semibold transition-all ${m.color}`}
            >
              <span>{m.label}</span>
        ...
        {/* Legend */}
        <div className="flex flex-wrap items-center justify-between gap-2 pt-2 text-[11px] text-slate-400 font-medium border-t border-slate-800/80">
